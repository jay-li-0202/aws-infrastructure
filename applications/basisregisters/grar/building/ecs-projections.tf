resource "aws_service_discovery_service" "projections" {
  name = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-projections"
...
resource "aws_ecs_service" "projections" {
  name            = "${var.app}-building-registry-projections"
  ...
  load_balancer {
    ...
    container_name   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-projections"
...
resource "aws_ecs_task_definition" "projections" {
  family                   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-projections"
  ...
  tags = {
    Name        = "Building Registry Projections // ${var.environment_label} ${var.environment_name}"
...
data "template_file" "projections" {
  ...
  vars = {
    ...
    app_name          = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-projections"
...
resource "aws_cloudwatch_metric_alarm" "projections_cpu_high" {
  alarm_name          = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-CPU-High-${var.ecs_as_cpu_high_threshold_per}-building-registry-projections"
