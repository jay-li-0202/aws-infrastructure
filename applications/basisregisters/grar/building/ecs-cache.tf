resource "aws_ecs_task_definition" "cache" {
  family                   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-cache"
...
data "template_file" "cache" {
  ...
  vars = {
    ...
    app_name         = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-cache"
...
resource "aws_iam_role" "ecs_events" {
  name        = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-ecs-events"
...
description         = "Run ${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-cache task at a scheduled time (${var.cache_schedule_expression})"
schedule_expression = var.cache_schedule_expression
}

resource "aws_cloudwatch_event_target" "cache" {
target_id = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-building-registry-cache"
