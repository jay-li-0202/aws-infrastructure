resource "aws_lb_target_group" "legacy" {
  ...
  tags = {
    Name        = "StreetName Legacy // ${var.environment_label} ${var.environment_name}"
...
resource "aws_lb_target_group" "extract" {
  ...
  tags = {
    Name        = "StreetName Extract // ${var.environment_label} ${var.environment_name}"
    Environment = var.tag_environment
    Productcode = var.tag_product
    Programma   = var.tag_program
    Contact     = var.tag_contact
...
resource "aws_lb_listener_rule" "extract" {
  ...
  condition {
    field  = "host-header"
    values = ["streetname-extract.*"]
  }
}

resource "aws_lb_target_group" "projections" {
  name                 = "streetname-projections"
  ...
  tags = {
    Name        = "StreetName Projections // ${var.environment_label} ${var.environment_name}"
    Environment = var.tag_environment
    Productcode = var.tag_product
    Programma   = var.tag_program
    Contact     = var.tag_contact
