resource "aws_service_discovery_service" "import-api" {
  name = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-import-api"
...
resource "aws_ecs_service" "import-api" {
  name            = "${var.app}-parcel-registry-import-api"
  ...
  load_balancer {
    ...
    container_name   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-import-api-import"
...
resource "aws_ecs_task_definition" "import-api" {
  family                   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-import-api"
  ...
  tags = {
    Name        = "Parcel Registry Import Api // ${var.environment_label} ${var.environment_name}"
...
data "template_file" "import-api" {
  ...
  vars = {
    ...
    app_name          = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-import-api"
