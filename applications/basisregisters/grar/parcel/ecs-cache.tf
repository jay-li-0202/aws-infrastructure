resource "aws_ecs_task_definition" "cache" {
  family                   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-cache"
  ...
  tags = {
    Name        = "Parcel Registry Cache // ${var.environment_label} ${var.environment_name}"
...
data "template_file" "cache" {
  ...
  vars = {
    ...
    app_name         = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-cache"
...
resource "aws_iam_role" "ecs_events" {
  name        = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-ecs-events"
...
name                = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-cache"
description         = "Run ${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-cache task at a scheduled time (${var.cache_schedule_expression})"
schedule_expression = var.cache_schedule_expression
}

resource "aws_cloudwatch_event_target" "cache" {
target_id = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-parcel-registry-cache"
