resource "aws_lb_target_group" "legacy" {
  ...
  tags = {
    Name        = "Address Legacy // ${var.environment_label} ${var.environment_name}"
...
resource "aws_lb_target_group" "extract" {
  ...
  tags = {
    Name        = "Address Extract // ${var.environment_label} ${var.environment_name}"
    Environment = var.tag_environment
    Productcode = var.tag_product
    Programma   = var.tag_program
    Contact     = var.tag_contact
...
resource "aws_lb_listener_rule" "extract" {
  ...
  condition {
    field  = "host-header"
    values = ["address-extract.*"]
  }
}

resource "aws_lb_target_group" "projections" {
  name                 = "address-projections"
  ...
  tags = {
    Name        = "Address Projections // ${var.environment_label} ${var.environment_name}"
    Environment = var.tag_environment
    Productcode = var.tag_product
    Programma   = var.tag_program
    Contact     = var.tag_contact
