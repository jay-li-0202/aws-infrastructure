resource "aws_service_discovery_service" "api" {
  name = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-address-registry-api"
...
resource "aws_ecs_service" "api" {
  name            = "${var.app}-address-registry-api"
  ...
  load_balancer {
    ...
    container_name   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-address-registry-api-legacy"
    container_port   = var.port_range + 2
  }

  load_balancer {
    target_group_arn = aws_lb_target_group.import.id
    container_name   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-address-registry-api-import"
    container_port   = var.port_range
  }

  load_balancer {
    target_group_arn = aws_lb_target_group.extract.id
    container_name   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-address-registry-api-extract"
...
resource "aws_ecs_task_definition" "api" {
  family                   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-address-registry-api"
  ...
  tags = {
    Name        = "Address Registry Api // ${var.environment_label} ${var.environment_name}"
...
data "template_file" "api" {
  ...
  vars = {
    ...
    app_name          = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-address-registry-api"
...
resource "aws_cloudwatch_metric_alarm" "api_cpu_high" {
  alarm_name          = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-CPU-High-${var.ecs_as_cpu_high_threshold_per}-address-registry-api"
...
resource "aws_cloudwatch_metric_alarm" "api_cpu_low" {
  alarm_name          = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-CPU-Low-${var.ecs_as_cpu_low_threshold_per}-address-registry-api"
...
resource "aws_appautoscaling_policy" "api_up" {
  name               = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-address-registry-api-scale-up"
...
resource "aws_appautoscaling_policy" "api_down" {
  name               = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-address-registry-api-scale-down"
  service_namespace  = aws_appautoscaling_target.api.service_namespace
  resource_id        = aws_appautoscaling_target.api.resource_id
  scalable_dimension = aws_appautoscaling_target.api.scalable_dimension
