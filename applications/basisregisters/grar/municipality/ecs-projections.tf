...
resource "aws_ecs_service" "projections" {
  ...
  load_balancer {
    ...
    container_name   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-municipality-registry-projections"
...
resource "aws_ecs_task_definition" "projections" {
  family                   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-municipality-registry-projections"
