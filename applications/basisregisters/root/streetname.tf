...
module "streetname-registry" {
  ...
  api_min_instances = 2
  api_max_instances = 4
  legacy_api_image  = "${var.aws_account_id}.dkr.ecr.eu-west-1.amazonaws.com/streetname-registry/api-legacy:1.14.5"
  import_api_image  = "${var.aws_account_id}.dkr.ecr.eu-west-1.amazonaws.com/streetname-registry/api-crab-import:1.14.5"
  extract_api_image = "${var.aws_account_id}.dkr.ecr.eu-west-1.amazonaws.com/streetname-registry/api-extract:1.14.5"

  projections_cpu           = 256
  projections_memory        = 1024
  projections_min_instances = 1
  projections_image         = "${var.aws_account_id}.dkr.ecr.eu-west-1.amazonaws.com/streetname-registry/projector:1.14.5"
  syndication_image         = "${var.aws_account_id}.dkr.ecr.eu-west-1.amazonaws.com/streetname-registry/projections-syndication:1.14.5"

  cache_cpu      = 256
  cache_memory   = 512
  cache_schedule = "cron(0/5 * * * ? *)"
  cache_enabled  = true
  cache_image    = "${var.aws_account_id}.dkr.ecr.eu-west-1.amazonaws.com//redis/redis-populator:1.5.2"
