resource "aws_cloudwatch_log_group" "app_log_group" {
  name              = "/ecs/task/${var.app}-${lower(replace(var.environment_name, " ", "-"))}-docs"
...
resource "aws_cloudwatch_log_stream" "app_log_stream" {
  name           = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-docs"
  log_group_name = aws_cloudwatch_log_group.app_log_group.name
}

resource "aws_cloudwatch_log_group" "monitoring_log_group" {
  name              = "/ecs/task/${var.app}-${lower(replace(var.environment_name, " ", "-"))}-docs-datadog"
  retention_in_days = 30

  tags = {
    Name        = "Docs Datadog // ${var.environment_label} ${var.environment_name}"
    Environment = var.tag_environment
    Productcode = var.tag_product
    Programma   = var.tag_program
    Contact     = var.tag_contact
...
resource "aws_cloudwatch_log_stream" "monitoring_log_stream" {
  name           = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-docs-datadog"
  log_group_name = aws_cloudwatch_log_group.monitoring_log_group.name
}

resource "aws_lambda_permission" "app_log" {
  statement_id  = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-docs"
