resource "aws_api_gateway_resource" "perceel_detail1" {
  ...
  parent_id   = aws_api_gateway_resource.percelen_root1.id
  path_part   = "{perceelId}"
}
...
resource "aws_api_gateway_method" "get-perceel1" {
  ...
  request_parameters = {
    "method.request.path.perceelId"           = true
...
resource "aws_api_gateway_integration" "get-perceel-integration1" {
  rest_api_id = var.rest_api_id
  resource_id = aws_api_gateway_resource.perceel_detail1.id
  http_method = aws_api_gateway_method.get-perceel1.http_method
  ...
  uri = "http://$${stageVariables.baseHost}/v1/percelen/{perceelId}"

  request_parameters = {
    "integration.request.path.perceelId"           = "method.request.path.perceelId"
