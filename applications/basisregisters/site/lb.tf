...
resource "aws_s3_bucket" "lb_access_logs" {
  bucket        = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-site-lb-access-logs"
  acl           = "private"
  force_destroy = true

  tags = {
    Name        = "Site Load Balancer Logs // ${var.environment_label} ${var.environment_name}"
    Environment = var.tag_environment
    Productcode = var.tag_product
    Programma   = var.tag_program
    Contact     = var.tag_contact
...
resource "aws_lambda_permission" "lb_access_logs" {
  statement_id  = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-site-lb-access-logs"
