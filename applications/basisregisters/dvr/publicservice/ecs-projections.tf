resource "aws_service_discovery_service" "projections" {
  name = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-publicservice-registry-projections"
...
resource "aws_ecs_service" "projections" {
  name            = "${var.app}-publicservice-registry-projections"
  ...
  load_balancer {
    ...
    container_name   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-publicservice-registry-projections"
...
resource "aws_ecs_task_definition" "projections" {
  family                   = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-publicservice-registry-projections"
  ...
  tags = {
    Name        = "PublicService Registry Projections // ${var.environment_label} ${var.environment_name}"
...
data "template_file" "projections" {
  ...
  vars = {
    ...
    app_name          = "${var.app}-${lower(replace(var.environment_name, " ", "-"))}-publicservice-registry-projections"
